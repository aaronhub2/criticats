...
function crearPeli() {
    ...
    fetch('https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=es&page=' + numPagina + '&sort_by=popularity.desc', options)
        .then(response => response.json())
        .then(response => {
            exitoRecogiendoPeli(response); // Call exitoRecogiendoPeli with the retrieved movie data
        })
        .catch(err => {
            console.log(err); // Call console log when there is an error
        });
}
...
window.addEventListener('scroll', function () {
    ...
    // Si el usuario está cerca del final de la página, crear nueva pelicula
