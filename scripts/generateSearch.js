...
function exitoRecogiendoPeli(data) {
    ...
    movies.forEach((movie, index) => {
        ...
        posterLink.href = 'movie.php?id=' + movie.id + '&tipo=0';
        ...
        tituloPeli.textContent = movie.title + ' (' + year + ')';
        ...
        getLists(index, movie.title, movie.id, 0);
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="reviewModalLabel${index}">Dejar una Reseña - ${movie.title}</h5>
                                        ...
                                        <div class="modal-body">
                                            <form id="reviewForm${index}" class="text-center" method="POST">
                                                ...
                                                <div class="form-group mb-3">
                                                    ...
                                                    <input hidden type="text" class="form-control" id="tipo${index}" name="content_id" value="0" readonly>
                                                </div>
                                                <div class="form-group mb-3">
...
function exitoRecogiendoTodo(data) {

    var numRes = data.results.length;

    for (var i = 0; i < numRes; i++) {
        var result = data.results[i];

        var tipoDato = result.media_type;

        if (tipoDato === "movie") {
            recogerPeliIndividual(result, i);
        } else if (tipoDato === "tv") {
            recogerSerieIndividual(result);
        } else {
            //Si no es movie o tv se ignora
        }
    }

}

function errorBuscando() {

    //Redirecciona a index en caso de error catastrófico
...
function onLoad() {
    ...
    // URLSearchParams recoge los datos de busqueda en la url
    ...
    if (typeValue == "Peliculas") {
        ...
        fetch('https://api.themoviedb.org/3/search/movie?query=' + searchValue + '&include_adult=false&language=es&page=1', options)
            .then(response => response.json())
            .then(response => exitoRecogiendoPeli(response))
            .catch(err => errorBuscando(err));
        ...
        fetch('https://api.themoviedb.org/3/search/tv?query=' + searchValue + '&include_adult=false&language=es&page=1', options)
            .then(response => response.json())
            .then(response => exitoRecogiendoSerie(response))
            .catch(err => errorBuscando(err));

        theMovieDb.common.api_key = 'c260b3e8954b170aed328ca4288bd341';


    } else if (typeValue == "Todo") {
        ...
        fetch('https://api.themoviedb.org/3/search/multi?query=' + searchValue + '&include_adult=false&language=es&page=1', options)
            .then(response => response.json())
            .then(response => exitoRecogiendoTodo(response))
            .catch(err => errorBuscando(err));
        ...
        //Redirecciona a index en caso de que no le llegue peli, serie o todo value
        console.log('error no llega filtro');
        //window.location.href = "index.php";
